...
LIMITS_DEFAULT = {0x1E: I_LIMITS, 0x20: T_LIMITS, 0x27: I_LIMITS, 0x29: T_LIMITS}
...
def pkt_gen(cmd_id, val=None, pkt_id=PKT_ID_DEFAULT):
    ...
    cmd_type = COMMAND_TYPES[cmd_id]
    data = num2bytes(val)
    # General packet format with len and chksum temporarily 0x00
    pkt = (
        bytearray([HDR, pkt_id, 0x00, ID_COMPUTER, ID_LASER, cmd_type, cmd_id])
        + data
        + b"\x00"
        + FTRCHR
    )
...
def pkt_read(ser):
    ...
    if len(s2) != remaining_bytes:
        raise IOError(
            "Insufficient bytes in serial stream to match packet size (%i vs %i)"
            % (3 + len(s2), remaining_bytes + 3)
        )
...
class OrionLaser:
    def __init__(self, port, name=None, verbose=False):
        ...
        try:
            ...
            self.i_factory = self.execute_cmd(
                0x04, None
            )  # Read factory default current (in 0.1 mA)
            self.t_factory = self.execute_cmd(
                0x06, None
            )  # Read factory default temp (ohms)

            self.i_invol = self.execute_cmd(
                0x26, None
            )  # Read involatile current (in 0.1 mA)
            self.t_invol = self.execute_cmd(0x28, None)  # Read involatile temp (ohms)

            self.i_0 = self.execute_cmd(
                0x1D, None
            )  # Read current setpoint at initialization (in 0.1 mA)
            self.t_0 = self.execute_cmd(
                0x1F, None
            )  # Read temp setpoint at initialization (ohms)
            ...
            print(
                "Failed initial configuration of ORION laser on %s: %s" % (self.port, e)
            )
            try:
                self.execute_cmd(0x25, None)  # Disable RS232
            except Exception:
    ...
    def execute_cmd(self, cmd, val):
        try:
            ...
            if val < lim[0] or val > lim[1]:
                raise ValueError(
                    "Requested value (%i) is outside limits (%i to %i)"
                    % (val, lim[0], lim[1])
                )
        except KeyError:
            pass
        ...
        if status_code != 0x00:
            ...
            print(
                "Reported set point (%i) does not match requested (%i)"
                % (return_value, val)
            )
        return return_value

...
#            self.file_output2.close()
    ...
    def initBuffer(self):
#        print('initBuffer')
        self.gate_time_counter = 100e6/self.sl.fs
        self.gate_time_dacs = 100e6/self.sl.fs/10
        try:
            self.N_history_counters = round(float(self.qedit_history.text()) / self.gate_time_counter)
            self.N_history_dacs = round(float(self.qedit_history.text()) / self.gate_time_dacs)
        except:
            self.N_history_counters = round(10 / self.gate_time_counter)
            self.N_history_dacs = round(10 / self.gate_time_dacs)
...
#                    print('Steps inhibited')
                ...
                if self.qchk_clear_temp_control.isChecked() == True:
                    ...
                    try:
                        ...
                        self.client.send_text('COMMITADJUST\n')
                        # Old method:
                        # self.client.send_text('%f\n' % self.setpoint_change)
                        self.qchk_clear_temp_control.setChecked(False)
                    except:
...
#                print('len = %d' % len(freq_counter_samples))
    ...
    #            self.time_history[-len(freq_counter_samples):] = time_axis
                ...
                self.qplt_freq.setTitle('Lock #%d Freq error, mean = %.6f Hz, std = %.3f mHz' % (self.output_number, np.mean(self.freq_history[self.bValid_counters]), 1e3*np.std(self.freq_history[self.bValid_counters])))

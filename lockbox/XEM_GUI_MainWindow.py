...
##        super(XEM_GUI_MainWindow, self).reject()
    ...
    def sendADCPhase(self):
        value = round(float(self.qtxt_adcphase.text()))
        self.sl.set_adc_clock_phase_shift_value(value)
...
#        if True:
    ...
    def showVNA(self):
        self.vna = DisplayVNAWindow()
        self.vna.sl = self.sl

    def readCEOBeatAmplitude(self, N_points=10e3):
        currentSelector= 'ADC0'
        ...
        try:
            ...
            complex_baseband = self.sl.frontend_DDC_processing(samples_out, ref_exp0, 0)
            amplitude = np.abs(complex_baseband)
            
            N_lowpass = 1e3+1
            amplitude_slow = smooth(amplitude, N_lowpass)
            amplitude_fast = amplitude - amplitude_slow[(N_lowpass-1)/2:-(N_lowpass-1)/2]
#            
#            plt.figure()
#            plt.plot(amplitude, '-')
#            plt.hold(True)
#            plt.plot(amplitude_fast, '-')
#            plt.plot(amplitude_slow, '-')
#            plt.title('Beat amplitude')
#            QtGui.QApplication.instance().processEvents()
            
            return (np.mean(amplitude), np.std(amplitude), np.std(amplitude_slow), np.std(amplitude_fast))
        except:
            print('exception occured in readBeatAmplitude()')
            raise
    ...
    def grabAndExportData(self):
        ...
        N_points_str, ok = QtGui.QInputDialog.getText(self, 'Raw data export', 
            'Enter the number of points desired [4e3, 64e6]:', Qt.QLineEdit.Normal, '64e6')
        if not ok:
            return
...
#            raise
    ...
    def setBytesSkip(self):
        self.sl.ddr_bytes_skip = int(self.qedit_bytes_skip.text())
...
#                    print('set slider to %d' % desired_ramp[k2])
    ...
    def ampPumpsRamping(self):
        # Parameters:
        
        

        
        # Init ILX communication:
        ilx = ILX_laser_control()
        
        
        # Init NI-DAQ communication:
#        nidaq = Instrument()
#        strDeviceNI = 'Dev1/ai3'
        
        if self.strFGPASerialNumber == '000000054J':
            pump1 = 3
            pump2 = 4
        else:
            pump1 = 1
        ...
        # Ask the user what the nominal optimum current for each pump is:
        try:
            
            pump1_current_sweep_range_str, ok = QtGui.QInputDialog.getText(self, 'Amplifiers pump optimization', 
                'Enter the sweep range for pump 1 in the format "Start, Stop, Number of points":', Qt.QLineEdit.Normal, '1400, 1500, 20')
            if not ok:
                return
            pump1_current_sweep_range_str = str(pump1_current_sweep_range_str).split(',')
            pump1_currents_to_try = np.linspace(float(pump1_current_sweep_range_str[0]), float(pump1_current_sweep_range_str[1]), int(float(pump1_current_sweep_range_str[2])))
            
            pump2_current_sweep_range_str, ok = QtGui.QInputDialog.getText(self, 'Amplifiers pump optimization', 
                'Enter the sweep range for pump 2 in the format "Start, Stop, Number of points":', Qt.QLineEdit.Normal, '1400, 1500, 20')
            if not ok:
                return
            pump2_current_sweep_range_str = str(pump2_current_sweep_range_str).split(',')
            pump2_currents_to_try = np.linspace(float(pump2_current_sweep_range_str[0]), float(pump2_current_sweep_range_str[1]), int(float(pump2_current_sweep_range_str[2])))
            
            
            pump1_current_guess_str, ok = QtGui.QInputDialog.getText(self, 'Amplifiers pump optimization', 
                'Enter the setpoint for pump 1 [1400 mA, 1500 mA]:', Qt.QLineEdit.Normal, '1470')
            if not ok:
                return
            pump1_current_guess = float(pump1_current_guess_str)
            
            pump2_current_guess_str, ok = QtGui.QInputDialog.getText(self, 'Amplifiers pump optimization', 
                'Enter the setpoint for pump 2 [1400 mA, 1500 mA]:', Qt.QLineEdit.Normal, '1470')
            if not ok:
                return
            pump2_current_guess = float(pump2_current_guess_str)
        except:
            

            print('Exception occured. using nominal current setpoints')
            raise
            return
            
        # Set pump2 at the optimum level:
        ilx.selectChannel(pump2)
        ilx.setLaserCurrent(pump2_current_guess)
#        time.sleep(1)
        
        self.ampPumpsRampingProcess(ilx, pump1, pump1_currents_to_try, nidaq=None, strDevice=None)
#        ilx.selectChannel(1)
        # Set pump 1 at the optimum level:
        ilx.setLaserCurrent(pump1_current_guess)
#        time.sleep(1)
        
        self.ampPumpsRampingProcess(ilx, pump2, pump2_currents_to_try, nidaq=None, strDevice=None)
        ilx.setLaserCurrent(pump2_current_guess)
        
    def ampPumpsRampingProcess(self, ilx, ilx_channel_number, pump_currents_to_try, nidaq, strDevice):
        ilx.selectChannel(ilx_channel_number)
        
        wait_time_for_first_setpoint = 5
        wait_time_per_setpoint = 2
        
        # Open a window:
        hFigSNR = plt.figure()
        plt.title('Beat SNR, channel %d' % ilx_channel_number)
        plt.xlim(pump_currents_to_try[0], pump_currents_to_try[-1])
        hFigSNR.canvas.draw()
        
        # Actual loop:
        qapp = QtGui.QApplication.instance()
        beat_amplitude = 0*pump_currents_to_try
        beat_noise = 0*pump_currents_to_try
        beat_noise_slow = 0*pump_currents_to_try
        beat_noise_fast = 0*pump_currents_to_try
        ceo_dc_power = 0*pump_currents_to_try
        k = 0
        for current_setpoint in pump_currents_to_try:
            
            print('Setting ILX current to %f mA...' % current_setpoint)
            ilx.setLaserCurrent(current_setpoint)
            if k == 0:
                time.sleep(wait_time_for_first_setpoint)
            else:
                time.sleep(wait_time_per_setpoint)
            
            print('Measuring beat amplitude...')
            (average_ceo_amplitude, ceo_amplitude_noise, ceo_amplitude_noise_slow, ceo_amplitude_noise_fast) = self.readCEOBeatAmplitude(N_points=1e6)
            beat_amplitude[k] = average_ceo_amplitude
            beat_noise[k] = ceo_amplitude_noise
            beat_noise_slow[k] = ceo_amplitude_noise_slow
            beat_noise_fast[k] = ceo_amplitude_noise_fast
            print('beat amplitude = %f' % average_ceo_amplitude)
            print('beat amplitude SNR = %f' % (average_ceo_amplitude/ceo_amplitude_noise))
            
            print('Measuring DC CEO power...')
#            data_all = nidaq.readData()
#            ceo_dc_power[k] = np.mean(data_all)
            if nidaq is not None:
                ceo_dc_power[k] = nidaq.getRawData(strDevice, N_points=1024)
            else:
                ceo_dc_power[k] = 0
            
            plt.clf()
            plt.plot(pump_currents_to_try, beat_amplitude/beat_noise, '.-', figure=hFigSNR)
            plt.xlim(pump_currents_to_try[0], pump_currents_to_try[-1])
            plt.ylim(ymin=0.)
            hFigSNR.canvas.draw()
        ...
        plt.figure()
        plt.plot(pump_currents_to_try, beat_amplitude, '.-')
        plt.title('Beat amplitude, channel %d' % ilx_channel_number)
        plt.ylim(ymin=0.)
        
#        plt.figure()
#        plt.plot(pump_currents_to_try, ceo_dc_power, '.-')
#        plt.title('CEO DC power, channel %d' % ilx_channel_number)
        
        plt.figure()
        plt.plot(pump_currents_to_try, beat_noise, '.-')
        plt.hold(True)
        plt.plot(pump_currents_to_try, beat_noise_fast, '.-')
        plt.plot(pump_currents_to_try, beat_noise_slow, '.-')
        plt.title('Beat amplitude noise, channel %d' % ilx_channel_number)
        plt.ylim(ymin=0.)
...
#        PalDarkerBackground.setColor(Qt.QPalette.Background, Qt.QColor(255, 255, 255))
        ...
        self.qbtn = QtGui.QPushButton('Export')
        ...
        self.qbtn_grab = QtGui.QPushButton('Grab and export')
        ...
        if self.selected_ADC == 0:
            ...
            self.qlabel_detected_vco_gain_label = Qt.QLabel('Detected VCO Gains [Hz/V]:')
        ...
        # DDR pipe bytes skip, for debug:
        self.qlabel_bytes_skip = Qt.QLabel('Bytes skip [bytes]:')
        self.qedit_bytes_skip = user_friendly_QLineEdit('0')
        self.qedit_bytes_skip.returnPressed.connect(self.setBytesSkip)
        self.qedit_bytes_skip.setMaximumWidth(60)
...
#            self.qtxt_adcphase = Qt.QLineEdit('380')
#            self.qbtn_send_adcphase = Qt.QPushButton('Send')
#            self.qbtn_send_adcphase.clicked.connect(self.sendADCPhase)
        elif self.selected_ADC == 1:
            self.qlbl_status1 = Qt.QLabel('Status: Idle')
...
#        self.qbtn_reset.clicked.connect(self.resetFrontend)
        ...
        # Button to initiate ramping of the amplifier's pumps:
        self.qbtn_amp_pumps = QtGui.QPushButton('Amp pumps')
        self.qbtn_amp_pumps.clicked.connect(self.ampPumpsRamping)
...
#        grid.addWidget(self.qedit_bytes_skip,           0, 4)
        ...
        if self.selected_ADC == 0:
            ...
            grid.addWidget(self.qbtn_amp_pumps,             0, 8, 1, 1)
...
#        self.qddc0_error_scale.setFillColor(Qt.Qt.blue)
        ...
        self.qlabel_rawdata_rbw = Qt.QLabel('RBW: 300 kHz')
        self.qedit_rawdata_length = Qt.QLineEdit('6e2')
...
#        self.qlabel_ddc_plot_select = Qt.QLabel('Plot type:')
        ...
        self.qlabel_ddc_rbw = Qt.QLabel('RBW: 100 kHz')
        self.qedit_ddc_length = Qt.QLineEdit('3e5')
        ...
        self.qedit_spc_averaging = Qt.QLineEdit('10')
...
#        self.timerIDDither = Qt
        ...
        self.timerIDDither.start(100)   # 100 ms readout delay
...
#            # Compute the running average:
            ...
            try:
                n_spc_avg = round(float(self.qedit_spc_averaging.text()))
                if n_spc_avg > 1.:
                    self.bAveragePhaseNoise = True
...
#                self.qplt_DDC0_spc.setAxisScale(Qwt.QwtPlot.xBottom, frequency_axis[1], frequency_axis[last_index_shown])
                ...
                integration_higher_index = round(integration_higher_bound/fs_new*len(spc))
...
#            self.curve_DDC0_spc.setData(frequency_axis[1:last_index_shown], spc[1:last_index_shown])
            ...
            if window_NEB > 1e6:
                self.qlabel_ddc_rbw.setText('RBW: %.1f MHz' % (round(window_NEB*1e5)/1e5/1e6))
            elif window_NEB > 1e3:
                self.qlabel_ddc_rbw.setText('RBW: %.1f kHz' % (round(window_NEB*1e2)/1e2/1e3))
            else:
                self.qlabel_ddc_rbw.setText('RBW: %.0f Hz' % (round(window_NEB)))
...
#            ref_exp = (ref_exp0)/np.abs(ref_exp0) * np.exp(-1j*2*math.pi*f_reference/self.sl.fs*np.array(range(len(samples_out))))
#            complex_baseband = samples_out * ref_exp
#            
#            N_max_IQ = 10e3 # Max number of points to display in the IQ graph
#            N_filter = 20
#            complex_baseband = lfilter(np.ones(N_filter, dtype=float)/N_filter, 1, complex_baseband)[N_filter:]

#            # Simulate a frequency divider by extracting the phase of the baseband signal, unwrapping, and then dividing by 64:            
#            phi = np.unwrap(np.angle(complex_baseband))
#            divided_beat = np.abs(complex_baseband) * np.exp(1j*phi/64. + 1j*2*np.pi*f_reference/self.sl.fs*np.array(range(len(complex_baseband))))
#            # Compute the spectrum of the divided beat:
#            window_function = np.blackman(len(divided_beat))
#            frequency_axis = np.linspace(0, (len(divided_beat)-1)/float(len(divided_beat))*(self.sl.fs/1e6), len(divided_beat))
#            last_index_shown = np.round(len(frequency_axis)/2)
#            spc = np.abs(np.fft.fft((divided_beat-np.mean(divided_beat)) * window_function))/(sum(window_function)/2)
#            spc = 20*np.log10(np.abs(spc) + 1e-6)
            
            # Update the graph data:
#            self.curve_divided.setData(frequency_axis[0:last_index_shown], spc[0:last_index_shown])
    ...
    #            self.qplt_spc.setAxisAutoScale(Qwt.QwtPlot.yLeft)
            ...
            complex_baseband = complex_baseband[:np.min((len(complex_baseband), N_max_IQ))]
